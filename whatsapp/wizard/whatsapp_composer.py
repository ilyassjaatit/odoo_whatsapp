...
class SendWhatsapp(models.TransientModel):
    ...
    def default_get(self, fields):
        result = super(SendWhatsapp, self).default_get(fields)
    ...
    composition_mode = fields.Selection([
        ...
        ('mass', 'Send whatsapp in batch')], string='Composition Mode',
    ...
    res_ids_count = fields.Integer(
        ...
        help='Number of recipients that will receive the Whatsapp if sent in mass mode, without applying the Active Domain value')
    comment_single_recipient = fields.Boolean(
        'Single Mode', compute='_compute_comment_single_recipient', compute_sudo=False,
        help='Indicates if the Whatsapp composer targets a single specific recipient')
    ...
    @api.depends_context('whatsapp_composition_mode')
    def _compute_composition_mode(self):
        for composer in self:
            if self.env.context.get('whatsapp_composition_mode') == 'guess' or not composer.composition_mode:
    ...
    def _compute_recipients(self):
        for composer in self:
            ...
            if records and issubclass(type(records), self.pool['mail.thread']):
                res = records._whatsapp_get_recipients_info(force_field=composer.number_field_name, partner_fallback=not composer.comment_single_recipient)
    ...
    def _compute_recipient_single(self):
        for composer in self:
            ...
            if not records or not issubclass(type(records), self.pool['mail.thread']) or not composer.comment_single_recipient:
                ...
                continue
            records.ensure_one()
            res = records._whatsapp_get_recipients_info(force_field=composer.number_field_name, partner_fallback=True)
            composer.recipient_single_description = res[records.id]['partner'].name or records._whatsapp_get_default_partners().display_name
    ...
    def action_send_whatsapp_mass_now(self):
    ...
    def _action_send_whatsapp(self):
        ...
        if self.composition_mode == 'numbers':
            ...
            if self.comment_single_recipient:
                return self._action_send_whatsapp_comment_single(records)
            else:
    ...
    def _action_send_whatsapp_numbers(self):
        # TODO Function that connects with whatsapp api
        raise "method not implemented"

    def _action_send_whatsapp_comment_single(self, records=None):
        ...
        return self._action_send_whatsapp_comment(records=records)
    ...
    def _action_send_whatsapp_mass(self, records=None):
        ...
        whatsapp_record_values = self._prepare_mass_whatsapp_values(records)
        whatsapp_all = self._prepare_mass_whatsapp(records, whatsapp_record_values)

        if whatsapp_all and self.mass_keep_log and records and issubclass(type(records), self.pool['mail.thread']):
            log_values = self._prepare_mass_log_values(records, whatsapp_record_values)
            records._message_log_batch(**log_values)

        if whatsapp_all and self.mass_force_send:
            whatsapp_all.filtered(lambda whatsapp: whatsapp.state == 'outgoing').send(auto_commit=False, raise_exception=False)
            return self.env['whatsapp.whatsapp'].sudo().search([('id', 'in', whatsapp_all.ids)])
        return whatsapp_all
    ...
    def _get_optout_record_ids(self, records, recipients_info):
        ...
        as no opt-out mechanism exist in Whatsapp, see Whatsapp Marketing. """
        return []
    ...
    def _prepare_recipient_values(self, records):
        recipients_info = records._whatsapp_get_recipients_info(force_field=self.number_field_name)
    ...
    def _prepare_mass_whatsapp_values(self, records):
        all_bodies = self._prepare_body_values(records)
        ...
        for record in records:
            ...
            if sanitized and record.id in blacklist_ids:
                state = 'canceled'
                failure_type = 'whatsapp_blacklist'
            elif sanitized and record.id in optout_ids:
                state = 'canceled'
                failure_type = 'whatsapp_optout'
            elif sanitized and record.id in done_ids:
                state = 'canceled'
                failure_type = 'whatsapp_duplicate'
            elif not sanitized:
                state = 'canceled'
                failure_type = 'whatsapp_number_format' if recipients['number'] else 'whatsapp_number_missing'
            else:
                state = 'outgoing'
                failure_type = ''
    ...
    def _prepare_mass_whatsapp(self, records, whatsapp_record_values):
        whatsapp_create_vals = [whatsapp_record_values[record.id] for record in records]
        return self.env['whatsapp.whatsapp'].sudo().create(whatsapp_create_vals)
    
    def _prepare_log_body_values(self, whatsapp_records_values):
        result = {}
        for record_id, whatsapp_values in whatsapp_records_values.items():
            result[record_id] = plaintext2html(html2plaintext(whatsapp_values['body']))
        return result
    
    def _prepare_mass_log_values(self, records, whatsapp_records_values):
        return {
            'bodies': self._prepare_log_body_values(whatsapp_records_values),
